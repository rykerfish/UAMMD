/*Raul P. Pelaez 2020. Potential and field correction for the Doubly Periodic
 * Poisson solver. Slab geometry
 */
...
__device__ cufftComplex2 analyticalCorrectionNonMetallic(double k, double z,
                                                         double H,
                                                         Permitivity perm,
                                                         MismatchVals mis) {
  ...
  const double denominator =
      (exp(double(-2.0) * H * k) * (double(-1.0) + eb + et - eb * et) +
       (double(1.0) + eb) * (double(1.0) + et));
  // This expression is written so the highest exponential evaluated is
  // exp(kmax*HE)
  auto ezm2H = exp(k * (z - double(2.0) * H)) * (double(-1.0) + et) *
               (mis.mE0 / perm.inside - eb * k * mis.mP0);
  auto emz = exp(-z * k) * (double(1.0) + et) *
             (mis.mE0 / perm.inside - eb * k * mis.mP0);
  auto emHmz = exp((-H - z) * k) * (double(-1.0) + eb) *
               (mis.mEH / perm.inside + et * k * mis.mPH);
  auto emHpz = exp(k * (-H + z)) * (double(1.0) + eb) *
               (mis.mEH / perm.inside + et * k * mis.mPH);
...
__device__ cufftComplex2 analyticalCorrectionOneMetallicWall(
    double k, double z, double H, Permitivity perm, MismatchVals mis,
    bool isMetallicTop) {
  if (isMetallicTop) {
    const double eb = perm.bottom / perm.inside;
    const double denominator =
        k * (exp(-k * H) * (1.0 - eb) + exp(k * H) * (1.0 + eb));
    const cufftComplex Ai =
        (-(1.0 + eb) * k * mis.mPH +
         exp(-k * H) * (mis.mE0 / perm.inside + eb * k * mis.mP0)) /
        denominator;
    const cufftComplex Bi =
        ((-1.0 + eb) * k * mis.mPH +
         exp(k * H) * (-mis.mE0 / perm.inside - eb * k * mis.mP0)) /
        denominator;
    cufftComplex2 EzAndPhi;
...
__device__ cufftComplex2 analyticalCorrectionTwoMetallicWalls(
    double k, double z, double H, Permitivity perm, MismatchVals mis) {
...
__device__ void fillCorrectionForWaveNumber(
    Permitivity perm, real k, real kmax, MismatchVals mis,
    cufftComplex2 linearModeCorrection, bool isMetallicTop,
    bool isMetallicBottom, real H, int Nz, real He, Iterator &correction) {
    ...
    if (abs(z) < halfH + He and k <= kmax) {
      ...
      if (k > 0) {
        ...
        if (not isMetallicBottom and not isMetallicTop)
          ...
          res = analyticalCorrectionOneMetallicWall(k, z, H, perm, mis,
                                                    isMetallicTop);
        correction[i] = res;
      } else {
...
__global__ void computeCorrectionD(cufftComplex2 *correctionRealSpace,
                                   MismatchPtr mismatch, int nkx, int nky,
                                   int nz, bool isMetallicBottom,
                                   bool isMetallicTop, real H, real2 Lxy,
                                   real kmax, real He, Permitivity perm) {
  ...
  auto corr = make_third_index_iterator(correctionRealSpace, ik.x, ik.y,
                                        Index3D(nkx / 2 + 1, nky, 2 * nz - 2));
  auto mis = mismatch.fetchMisMatch(ik, {nkx, nky});
  fillCorrectionForWaveNumber(perm, k, kmax, mis,
                              mismatch.linearModeCorrection[0], isMetallicTop,
                              isMetallicBottom, H, nz, He, corr);
}
...
__global__ void scaleFFTToForwardChebyshevTransform(cufftComplex2 *signal,
                                                    int nz, int nbatch) {
  const int id = blockIdx.x * blockDim.x + threadIdx.x;
  if (id >= nbatch * nz)
    return;
...
__global__ void
computeFullCorrectionXYD(const cufftComplex2 *analyticalCorrection,
                         cufftComplex4 *fullCorrection, int nkx, int nky,
                         int nz, real2 Lxy) {
  ...
  const auto corr = make_third_index_iterator(analyticalCorrection, ik.x, ik.y,
                                              Index3D(nkx / 2 + 1, nky, nz));
  auto fullCorr = make_third_index_iterator(fullCorrection, ik.x, ik.y,
                                            Index3D(nkx / 2 + 1, nky, nz));
  ...
  fori(0, nz) {
    ...
    fullCorr[i].x =
        cufftComplex({k.x * potential.y, -k.x * potential.x}) * isPairedX;
    fullCorr[i].y =
        cufftComplex({k.y * potential.y, -k.y * potential.x}) * isPairedY;
    fullCorr[i].z = Ez;
    fullCorr[i].w = potential;
  }
}

cached_vector<cufftComplex4>
computeFullCorrectionXY(cached_vector<cufftComplex2> &analyticalCorrection,
                        real3 boxSize, int3 n) {
  System::log<System::DEBUG>("Correction XY");
  real2 Lxy = make_real2(boxSize);
  cached_vector<cufftComplex4> fullCorrection((n.x / 2 + 1) * n.y *
                                              (2 * n.z - 2));
  auto analyticalCorrection_ptr =
      thrust::raw_pointer_cast(analyticalCorrection.data());
  auto fullCorrection_ptr = thrust::raw_pointer_cast(fullCorrection.data());
  int blockSize = 128;
  int nblocks = ((n.x / 2 + 1) * n.y) / blockSize + 1;
  CudaCheckError();
  computeFullCorrectionXYD<<<nblocks, blockSize>>>(
      analyticalCorrection_ptr, fullCorrection_ptr, n.x, n.y, n.z, Lxy);
  CudaCheckError();
...
public:
  Correction(Permitivity perm, int3 cellDim, real2 Lxy, real H, real He)
    ...
    CufftSafeCall(cufftPlanMany(&cufft_plan_forward, 1, &size, &size, stride,
                                dist, &size, stride, dist,
                                CUFFT_Complex2Complex<real>::value, batch));
    this->isMetallicTop = std::isinf(perm.top);
    this->isMetallicBottom = std::isinf(perm.bottom);
  }

  ~Correction() { CufftSafeCall(cufftDestroy(cufft_plan_forward)); }
  ...
  void correctSolution(cached_vector<cufftComplex4> &insideSolution,
    ...
    auto mismatchData = mismatch->compute(insideSolution, outsideSolution,
                                          surfaceValuesFourier, st); // Step 4,5
    System::log<System::DEBUG>("Correction");
    real3 boxSize = make_real3(Lxy, H);
    auto analyticalCorrection =
        computeAnalyticalCorrectionFourierSpace(mismatchData, st); // Step 4
    takeAnalyticalCorrectionToChebyshevSpace(analyticalCorrection, st); // Step
                                                                        // 4
    auto fullCorrection = computeFullCorrectionXY(analyticalCorrection, boxSize,
                                                  cellDim); // Step 4
    sumCorrectionToInsideSolution(fullCorrection, insideSolution,
                                  st); // Step 5, 6
  }

private:
  cached_vector<cufftComplex2>
  computeAnalyticalCorrectionFourierSpace(MismatchPtr mis, cudaStream_t st) {
    System::log<System::DEBUG>("Analytical Correction");
    const int3 n = cellDim;
    cached_vector<cufftComplex2> analyticalCorrection((n.x / 2 + 1) * n.y *
                                                      (2 * n.z - 2));
    auto corr = thrust::raw_pointer_cast(analyticalCorrection.data());
    // kmax must be chosen carefully so that there is no overflow in
    // exp(kmax*HE) when evaluating the correction Maximum exponents for an
    // exponential that do not result in inf:
    ...
    int blockSize = 128;
    int nblocks = ((n.x / 2 + 1) * n.y) / blockSize + 1;
    computeCorrectionD<<<nblocks, blockSize, 0, st>>>(
        corr, mis, n.x, n.y, n.z, isMetallicBottom, isMetallicTop, H, Lxy, kmax,
        He, permitivity);
    CudaCheckError();
    return analyticalCorrection;
  }

  void takeAnalyticalCorrectionToChebyshevSpace(
      cached_vector<cufftComplex2> &analyticalCorrection, cudaStream_t st) {
    System::log<System::DEBUG>("Analytical Correction to Chebyshev");
    cufftComplex *d_data =
        (cufftComplex *)thrust::raw_pointer_cast(analyticalCorrection.data());
    const int blockSize = 128;
    const int3 n = cellDim;
    int nblocks = (((n.x / 2 + 1)) * n.y * n.z) / blockSize + 1;
    detail::periodicExtension<<<nblocks, blockSize, 0, st>>>(
        (cufftComplex2 *)d_data, n.z, (n.x / 2 + 1) * n.y);
    CufftSafeCall(cufftSetStream(cufft_plan_forward, st));
    CufftSafeCall(cufftExecComplex2Complex<real>(cufft_plan_forward, d_data,
                                                 d_data, CUFFT_FORWARD));
    nblocks = ((n.x / 2 + 1) * n.y * n.z) / blockSize + 1;
    detail::scaleFFTToForwardChebyshevTransform<<<nblocks, blockSize, 0, st>>>(
        (cufftComplex2 *)d_data, n.z, (n.x / 2 + 1) * n.y);
    CudaCheckError();
  }
};
